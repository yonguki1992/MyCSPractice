...
namespace LambdaFuncEx
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //무명 메서드를 람다식으로 표현한 경우
            //람다식의 우측항이 한 문장일 경우 {} 를 생략해도 됨.
            this.btn1.Click += (obj, args) => { ((Button)obj).BackColor = Color.Aqua; };
            this.btn2.Click += (obj, args) => ((Button)obj).BackColor = Color.IndianRed;
        ...
        // 위 함수의 람다식
        private SumDelegate sumDel2 = new SumDelegate((a, b) => a + b);
        ...
        private void MyMethod()
        {
            ...
            for(int i = 0; i < array.Length; ++i)
            {
                ...
                do
                {
                    tmp = rand.Next(array.Length);
                } while (array[tmp] != 0);
                array[tmp] = 1;
                numbers[i] = tmp;
            }

            // Array.Foreach의 Action 부분도 람다식으로 변경 가능할 듯

            //Array.ForEach(numbers,
            //    new Action<int>(val => textBox1.Text += val + Environment.NewLine));

            //위 내용 줄여서 쓴것
            Array.ForEach(numbers, val => textBox1.Text += val + Environment.NewLine);
